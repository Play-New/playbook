...
- **Feeling alignment:** read the target feeling from CLAUDE.md. Does this screen's first impression (the sensation in the first 500ms, before reading any text) match the target feeling? If the target is "calm control", does the screen feel calm? If it's "warm discovery", does it feel warm? This test precedes all others — a screen that nails every token but misses the feeling has failed.
...
- **Experience pattern compliance:** if `.superskills/design-system.md` has Experience Patterns, check: are micro-interactions present on user actions? Are transitions appropriate? Are gratification moments proportional? Has the absence test been applied (every visible element survives removal)?
